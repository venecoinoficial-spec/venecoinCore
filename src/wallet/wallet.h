...
/* 1.14.5: set the wallet's discard threshold to 1 Venecoin because that's what 97%
 ...
 */
/* 1.14.6: set the wallet's discard threshold to 0.01 Venecoin. Very network
...
/*
 * venecoin: Scaled to 1/10th of the recommended transaction fee to make RBF
 ...
 */
static const CAmount VENECOIN_WALLET_INCREMENTAL_RELAY_FEE = RECOMMENDED_MIN_TX_FEE / 10;
/*
 * venecoin: Creating change outputs at exactly the dustlimit is counter-
 ...
 *
 * Using bumpfee currently will add VENECOIN_WALLET_INCREMENTAL_RELAY_FEE as a fixed
...
public:
    ...
    /**
     * venecoin: Get a fee targeting a specific transaction speed.
     */
    CAmount GetvenecoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * venecoin: Get a fee targeting a specific transaction speed.
     */
    static CAmount GetvenecoinPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the venecoin dust fee.
     */
