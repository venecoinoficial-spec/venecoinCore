...
// venecoin -  1.14 specific fix: do not request headers from a peer we are
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    else if (strCommand == NetMsgType::INV)
    {
        ...
        if (best_block != nullptr) {
            // venecoin: allow header requests from inv to be non-serial by
    ...
    else if (strCommand == NetMsgType::GETHEADERS)
    {
        ...
        LOCK(cs_main);
        // VENECOIN: Allow getheaders during IBD if we're syncing from genesis or peer is whitelisted
        if (IsInitialBlockDownload() && !pfrom->fWhitelisted && !GetBoolArg("-allowgenesissync", true)) {
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (mapBlockIndex.find(headers[0].hashPrevBlock) == mapBlockIndex.end() && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            // venecoin: allow a single getheaders query before triggering DoS
            RequestHeadersFrom(pfrom, connman, pindexBestHeader, uint256(), true);
        ...
        if (nCount == MAX_HEADERS_RESULTS) {
            ...
            // from there instead.
            //
            // venecoin: do not allow multiple getheader queries in parallel at
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus(chainActive.Height());
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                // venecoin: make sure that if we are already processing an inv
        ...
        //
        std::vector<CInv> vGetData;
        // VENECOIN: Allow block download during IBD if we're syncing from genesis
        bool allowIBDDownload = GetBoolArg("-allowgenesissync", true) || GetBoolArg("-forcesync", false);
        if (!pto->fClient && (fFetch || !IsInitialBlockDownload() || allowIBDDownload) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
