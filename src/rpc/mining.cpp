...
{
    // venecoin: Never mine witness tx
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated venecoin to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0 && GetBoolArg("-requireconnections", false))
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "venecoin is not connected!");

    if (IsInitialBlockDownload() && GetBoolArg("-requiresync", false))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "venecoin is downloading blocks...");

    // Anti-hardware mining control system
    if (!GetBoolArg("-allowpowerful", false)) {
        static std::map<std::string, std::pair<int64_t, int>> minerStats;
        std::string minerID = request.strMethod + "_" + std::to_string(request.id.get_int() % 1000);
        ...
        if (!minerID.empty()) {
            ...
            if (it != minerStats.end()) {
                int64_t timeDiff = currentTime - it->second.first;
                int requestCount = it->second.second + 1;
                
                // Limit: 13 requests per minute (~1,300 KH/s)
                if (timeDiff > 0 && timeDiff < 60 && requestCount > 13) {
                    throw JSONRPCError(RPC_MISC_ERROR, "Miner too powerful: " + std::to_string(requestCount) + " requests/min from " + minerID + " (Max: 13 req/min = ~1,300 KH/s). Use allowpowerful=1 to bypass.");
                }
                
                // Reset counter every minute
                if (timeDiff >= 60) {
                    minerStats[minerID] = std::make_pair(currentTime, 1);
                } else {
                    minerStats[minerID] = std::make_pair(it->second.first, requestCount);
                }
            } else {
                minerStats[minerID] = std::make_pair(currentTime, 1);
            }
            ...
            for (auto iter = minerStats.begin(); iter != minerStats.end();) {
                if (currentTime - iter->second.first > 300) {
                    iter = minerStats.erase(iter);
                } else {
                    ++iter;
                }
            }
        ...
        fLastTemplateSupportsSegwit != fSupportsSegwit)
    {
        ...
        if (!coinbaseScript || coinbaseScript->reserveScript.empty())
            throw JSONRPCError(RPC_WALLET_KEYPOOL_RAN_OUT, "Error: Keypool ran out, please call keypoolrefill first, or unlock your wallet.");

        pblocktemplate = BlockAssembler(Params()).CreateNewBlock(coinbaseScript->reserveScript, fMineWitnessTx);
        if (!pblocktemplate)
