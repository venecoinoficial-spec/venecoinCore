...
#if defined(NDEBUG)
# error "venecoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "venecoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetvenecoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool IsInitialBlockDownload()
{
    ...
    // VENECOIN: Allow forced sync for public nodes
    if (GetBoolArg("-forcesync", false)) {
        return false;
    }
    ...
    if (chainActive.Tip() == NULL) {
        // VENECOIN: Allow sync from genesis if we have peers with blocks
        if (GetBoolArg("-allowgenesissync", true)) {
            return false;
        }
        return true;
    }
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // venecoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("venecoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // venecoin: BIP30 has been active since inception
    ...
    // venecoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetvenecoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // venecoin: Disallow AuxPow blocks before it is activated.
    ...
    // venecoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // venecoin: We probably want to disable this
    int nLockTimeFlags = 0;
