venecoin Core version 1.14.6 is now available from:

  <https://github.com/venecoin/venecoin/releases/tag/v1.14.6/>

This is a new minor version release, including important security updates and
changes to network efficiency. All venecoin Core users - miners, services,
...
venecoin network from 1 DOGE to 0.01 DOGE. The full recommendation can be found
...
```
venecoin-cli addnode <address> remove
```
...
```
venecoin-cli liststucktransactions
```
...
```
venecoin-cli setmaxconnections <desired_amount>
```
...
```
venecoin-cli rescan <start_block>
```
...
efforts to make venecoin more accessible to people that do not speak English.
...
* Improved and updated the [bip list](bips.md) documentation that explains the
  protocol features implemented by venecoin Core.
